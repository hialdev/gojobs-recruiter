<template>
    <div class="relative">
        <div :class="['cursor-pointer bg-white p-2 px-4 rounded-4xl flex justify-between items-center gap-3 text-gray-800 rounded-3xl w-full text-slate-400',$parent.class]"
            @click="toggleModal"
            ref="dropdownTrigger"
        >
            ...
            <div class="me-auto text-slate-500">
                <div v-if="selecteds.length > 0" class="flex items-center gap-1">
                    <div v-for="selected in selecteds.slice(0, 2)" :key="selected?.key" class="bg-emerald-100 text-emerald-700 rounded-3xl px-2">
                        {{ selected?.value }}
                    </div>
                    <div v-if="selecteds.length > 2" class="bg-emerald-100 text-emerald-700 rounded-3xl px-2">
                        +{{ selecteds.length - 2 }}
                    </div>
                </div>
                <div v-else>
        ...
        <div ref="modalElement" v-if="isModalOpen" :class="['modal absolute top-0 w-full mt-[3.5em] shadow rounded-xl',modalClass]">
            <ul 
                class="p-3 bg-white rounded-xl max-h-[20em] overflow-auto"
            >
                <li class="mb-3"><PartialFormSearch :label="'Cari '+label" :customClass="'w-full lg:max-w-[25em]'" @input="searchOptions" /></li>
                <div v-for="option in filteredOptions"
                    ...
                    >
                        <div class="cursor-pointer flex items-center justify-between gap-3">
                            ...
                            <div :class="{'text-emerald-500':selecteds.includes(option), 'text-slate-300':!selecteds.includes(option)}" class="w-[20px]">
