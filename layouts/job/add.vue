    ...
    <NuxtLayout name="default">
        <div class="container mx-auto p-5">
            <div class="flex items-center justify-between mb-5">
                <NuxtLink to="/job" class="flex items-center gap-4">
                    ...
                    <div class="font-medium">Tambah Pekerjaan Baru</div>
                </NuxtLink>
                        ...
                        <div v-if="showStatus" class="absolute top-0 end-0 mt-[2.3em] z-10">
                            <div class="bg-white p-5 rounded-2xl min-w-[10em]">
                                <nav class="flex flex-col items-start gap-1">
                                    <button v-for="sts in status" :key="sts.key" @click="selectedStatus = sts.value" :class="{'bg-[#DCF7E1]' : selectedStatus === sts.value}" class="text-start cursor-pointer text-emerald-900 p-3 w-full rounded-lg text-[#41806D] text-sm hover:bg-[#DCF7E1]">{{sts.value}}</button>
                                </nav>
                            </div>
                        </div>
                    </div>
                    <button class="px-4 py-2 text-sm bg-emerald-700 text-white rounded-lg">Save Job</button>
                    <button @click="navigateTo('/job/add/setting')" class="px-4 py-2 text-sm ring-2 ring-inset ring-slate-200 text-slate-700 rounded-lg flex items-center gap-2">
            ...
            <div class="bg-white rounded-xl">
                <div class="p-3 border-b-[4px] mb-5 border-slate-50">
                    <div class="box">
                        <NuxtLink to="/job/add/1" activeClass="activeStep" class="relative box-item" :class="{'filled': addStep > 1}">
                        ...
                        <NuxtLink to="/job/add/2" activeClass="activeStep" class="relative box-item" :class="{'filled': addStep > 2}">
                            <div class="absolute flex items-center gap-4 p-2 ps-6 top-0 bottom-0 start-0 end-0">
                        ...
                        <NuxtLink to="/job/add/3" activeClass="activeStep" class="relative box-item" :class="{'filled': addStep > 3}">
                            <div class="absolute flex items-center gap-4 p-2 ps-6 top-0 bottom-0 start-0 end-0">
                        ...
                        <NuxtLink to="/job/add/4" activeClass="activeStep" class="relative box-item" :class="{'filled': addStep === 'success'}">
                            <div class="absolute flex items-center gap-4 p-2 ps-6 top-0 bottom-0 start-0 end-0">
...
watch(
    ...
    () => {
        ...
        addStep.value = pathSegments[pathSegments.length - 1];  
    }
)
</script>
